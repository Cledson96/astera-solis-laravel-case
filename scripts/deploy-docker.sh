...
DEPLOY_PATH="${DEPLOY_PATH:-/opt/astera-solis}"
DEPLOY_ENVIRONMENT="${DEPLOY_ENVIRONMENT:-production}"
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-astera-solis-production}"
...
API_DOMAIN="${API_DOMAIN:?API_DOMAIN obrigatoria}"
...
publish_nginx_config() {
  ...
  printf '\n' >> "${temp_config}"
  render_domain_config "${API_DOMAIN}" "${API_PORT}" >> "${temp_config}"
...
compose() {
  sudo_run env COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME}" FRONTEND_PORT="${FRONTEND_PORT}" API_PORT="${API_PORT}" docker compose --env-file "${DEPLOY_PATH}/.env" -f "${COMPOSE_FILE}" "$@"
}

print_compose_logs() {
  compose ps || true
  compose logs --tail=120 api web worker || true
}
...
if [ "${FRONTEND_DOMAIN}" = "${API_DOMAIN}" ]; then
  log "FRONTEND_DOMAIN e API_DOMAIN devem ser diferentes"
  exit 1
fi
...
select_runtime_port web 3000 "${FRONTEND_PORT}"
FRONTEND_PORT="${SELECTED_PORT}"
select_runtime_port api 8000 "${API_PORT}"
...
issue_certificate_if_missing "${API_DOMAIN}"
publish_nginx_config
reload_nginx

wait_for_url api "http://127.0.0.1:${API_PORT}/up"
wait_for_url web "http://127.0.0.1:${FRONTEND_PORT}/"
